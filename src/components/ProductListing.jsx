...
const ProductListing = ({ setPage, setProductId, searchQuery }) => {
  ...
  // ===== Products ab Django API se aate hain (hardcoded nahi) =====
  ...
  // Jab search badle to wapas page 1 pe aa jao
  useEffect(() => {
    setPageNum(1);
  }, [searchQuery]);
  ...
  useEffect(() => {
    ...
    fetch(`https://muhammadbilal786.pythonanywhere.com/api/products/?${params.toString()}`)
      .then((res) => res.json())
      .then((data) => {
        ...
        const mapped = results.map((p) => ({
          ...
          price: p.price,
          desc: p.description,
          image: p.image || laptopImg, // image null ho to fallback
          rating: 4.5,
          orders: p.stock,
      ...
      <div className="flex gap-6">
        ...
        <main className="flex-1">
          ...
          <div className="bg-white border border-[#DEE2E7] rounded-lg p-4 flex items-center justify-between mb-4">
            <span className="text-[#1C1C1C] text-sm">{totalCount} items <span className="font-bold">found</span></span>
          ...
          {loading ? (
            <div className="text-center text-[#8B96A5] py-20">Loading products...</div>
          ) : viewMode === 'list' ? (
            /* Product List View */
            <div className="space-y-3">
              {products.map((product) => (
                <div key={product.id} className="bg-white border border-[#DEE2E7] rounded-lg p-5 flex gap-6 hover:shadow-md transition-shadow group cursor-pointer relative" onClick={() => { setProductId(product.id); setPage('details'); }}>
                  {/* Product Image area */}
                  ...
                  <div className="flex-1 py-1">
                    ...
                    <div className="flex items-center gap-4 mb-3">
                      <div className="flex flex-col">
                        <span className="text-xl font-bold text-[#1C1C1C]">Rs {product.price}</span>
                        {product.oldPrice && <span className="text-[#8B96A5] line-through text-sm mt-0.5">Rs {product.oldPrice}</span>}
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.map((product) => (
                <div
                  ...
                  onClick={() => { setProductId(product.id); setPage('details'); }}
                >
                  {/* Product Image Area */}
                  ...
                  <div className="w-full">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex flex-col">
                        ...
                        {product.oldPrice && <span className="text-[#8B96A5] line-through text-xs">Rs {product.oldPrice}</span>}
                      </div>
                    </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-end mt-8">
              <div className="flex border border-[#DEE2E7] rounded-md overflow-hidden bg-white">
                <button
                  className="px-3 py-2 border-r border-[#DEE2E7] text-dark hover:bg-shade transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  disabled={pageNum === 1}
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    className={`px-4 py-2 border-r border-[#DEE2E7] text-sm transition-colors ${p === pageNum ? 'bg-primary text-white font-bold' : 'text-dark hover:bg-shade'}`}
                  >
                    {p}
                  </button>
                ))}
                <button
                  className="px-3 py-2 text-dark hover:bg-shade transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
